...
/*
	...
	$(document).on('click', '.third-step', function(){
		...
		doc.text(10, 25, '________________________________________________________________');	
		doc.setFontSize(12);
		doc.text(20, 35, 'J: Provider Video means all audio, video, and/or audiovisual content to which Provider holds');
		doc.setFontSize(12);
		doc.text(20, 40, 'rights for during the Term.');
		doc.setFontSize(12);
		doc.text(20, 50, 'K. Claimed Video means Provider Video, Titles, Live Event Content, and Archived ');
		doc.setFontSize(12);	
		doc.text(20, 55, 'Content posted by Provider and/or users on, and accessible via the internet and Claimed ');
		doc.setFontSize(12);	
		doc.text(20, 60, 'by Broadcast Media for display on any device including but not limited to personal ');
		doc.setFontSize(12);	
		doc.text(20, 65, 'computers, tablets(e.g. iPad), Connected-TVs, smartphones, and similar devices (e.g. iPhone,');
		doc.setFontSize(12);	
		doc.text(20, 70, 'Android, Blackberry devices).');
		doc.setFontSize(12);	
		doc.text(20, 80, 'L. CPM means cost per thousand views of Broadcast Media Ads associated with Titles and/or');
		doc.setFontSize(12);	
		doc.text(20, 85, 'N. Provider Content means all Titles, Provider Videos, Archived Content, and Provider');
		doc.setFontSize(12);	
		doc.text(20, 95, 'Channels, whether created by or for Provider, including but not limited to graphics, ');
		doc.setFontSize(12);	
		doc.text(20, 100, 'music, sounds,images, files, photos, animation, artwork, text, data,information, ');
		doc.setFontSize(12);	
		doc.text(20, 105, 'messages, hypertext,links,scripts, or other materials.');
		doc.setFontSize(15);	
		doc.text(90, 115, 'LICENSE');
		doc.setFontSize(12);	
		doc.text(20, 120, 'Media Owner hereby grants to ArkonDigital the exclusive, irrevocable and worldwide right and');
		doc.setFontSize(12);	
		doc.text(20, 125, 'license to use, reproduce, keep copies of for archival purposes, promote, offer to sell, sell,');
		doc.setFontSize(12);	
		doc.text(20, 130, 'distribute, license, sub-license, publish, exhibit, display, perform, create derivate works,');
		doc.setFontSize(12);	
		doc.text(20, 135, 'and commercially exploit the Media, in part or in whole, for any purpose that Arkon,in its sole');
		doc.setFontSize(12);	
		doc.text(20, 140, 'discretion, considers appropriate at any time and from time to time. Without limiting the ');
		doc.setFontSize(12);	
		doc.text(20, 145, 'generality of the foregoing, the license herein granted will include the exclusive right of');
		doc.setFontSize(12);	
		doc.text(20, 150, 'ArkonDigital and its sublicencees:');
		doc.setFontSize(12);	
		doc.text(20, 160, 'a. to reproduce, modify, edit and transcode the Media, in part or in whole, as ArkonDigital');
		doc.setFontSize(12);	
		doc.text(20, 165, 'sees fit;');
		doc.setFontSize(12);	
		doc.text(20, 175, 'b. to publicize, advertise, market and promote the Media;');
		doc.setFontSize(12);	
		doc.text(20, 185, 'c. to use the names, photographs, likenesses, recorded voices and images of all performing');
		doc.setFontSize(12);	
		doc.text(20, 190, 'artists in and contributors to the Media in connection with the use,distribution, promotion and');
		doc.setFontSize(12);	
		doc.text(20, 195, 'advertising thereof.');
		doc.setFontSize(15);	
		doc.text(90, 205, 'WARRANTY');
		doc.setFontSize(12);	
		doc.text(20, 210, 'YOU AGREE TO INDEMNIFY, DEFEND AND HOLD HARMLESS ArkonDigital or Broadcast');
		doc.setFontSize(12);	
		doc.text(20, 215, ' Media FOR ANY CLAIMS, DAMAGES, LOSSES OR EXPENSES (INCLUDING REASONABLE');
		doc.setFontSize(12);	
		doc.text(20, 220, 'ATTORNEY FEES) IN ADDITION, YOU AGREE TO INDEMNIFY, DEFEND AND HOLD');
		doc.setFontSize(12);	
		doc.text(20, 225, 'HARMLESS ArkonDigital and Broadcast Media FOR ANY CLAIMS, DAMAGES, LOSSES');
		doc.setFontSize(12);	
		doc.text(20, 230, 'OR EXPENSES  ARISING OUT OF OR RESULTING FROM ANY INACCURACY,');
		doc.setFontSize(12);	
		doc.text(20, 235, 'MISSTATEMENT,CONTROVERSY,VIOLATION OF LAW,OR GOVERNMENTAL');
		doc.setFontSize(12);	
		doc.text(20, 240, 'INVESTIGATION RELATED TO THE MEDIA');
		doc.setFontSize(15);
		doc.text(90, 250, 'ROYALTIES');
		doc.setFontSize(12);	
		doc.text(20, 255, 'In consideration of the representations, warranties and license herein granted, Broadcast ');
		doc.setFontSize(12);	
		doc.text(20, 260, 'Media will pay to Media Owner royalties equal to Thirty percent ( 30 %) of the net Revenues,');
		doc.setFontSize(12);	
		doc.text(20, 265, 'To be paid and delivered within thirty (30) days from the moment ArkonDigital receives the ');	
		doc.setFontSize(15);
		doc.text(10, 270, '________________________________________________________________');
		doc.setFontSize(12);	
		...
		doc.text(10, 25, '________________________________________________________________');
		doc.setFontSize(12);	
		doc.text(20, 35, 'respective invoice based on the usage report provided by BroadCast Media. The date of');	
		doc.setFontSize(12);
		doc.text(20, 40, 'charge-off of money from the account of BroadCast Media is deemed date of payment. All');	
		doc.setFontSize(12);
		doc.text(20, 45, 'invoices are drawn and all payments are effected in CAD Dollars. Any money payable is');
		doc.setFontSize(12);
		doc.text(20, 50, 'less than CAD $1.00 in any given');	
		doc.setFontSize(15);	
		doc.text(10, 270, '________________________________________________________________');
		doc.setFontSize(12);	
		doc.text(10, 280, 'ArkonDigital Content Provider Agreement');
